...
///
/// This service polls for blockchain transaction confirmations and syncs
/// events back to the PostgreSQL database for analytics and audit purposes.
...
impl EventProcessorService {
    ...
    async fn start_websocket_listener(&self) {
        let mint = self.energy_token_mint.clone();
        ...
        info!(
            "Starting WebSocket listener for mint: {} on {}",
            mint, ws_url
        );
            ...
            loop {
                match PubsubClient::new(&ws_url).await {
                    Ok(pubsub_client) => {
                        ...
                        let filter = RpcTransactionLogsFilter::Mentions(vec![mint.clone()]);
                        ...
                        match pubsub_client.logs_subscribe(filter, config).await {
                            Ok((mut stream, _unsubscribe)) => {
                                info!("Subscribed to logs for mint: {}", mint);

                                while let Some(response) = stream.next().await {
                                    let signature = response.value.signature;
                                    debug!("Received log for signature: {}", signature);

                                    // Trigger confirmation check
                                    // We don't await here to avoid blocking the stream
                                    let service_clone = service.clone();
                                    tokio::spawn(async move {
                                        if let Err(e) =
                                            service_clone.confirm_transaction(&signature).await
                                        {
                                            warn!(
                                                "Error confirming transaction from WebSocket {}: {}",
                                                signature, e
                                            );
                                        }
                                    });
        ...
        loop {
                ...
                .get_transaction(&signature, UiTransactionEncoding::Json)
            {
                ...
                Err(e) => {
                    ...
                    backoff = std::cmp::min(backoff * 2, Duration::from_secs(5)); // Cap at 5s
                }
            }
        }
    }
