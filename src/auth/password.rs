...
impl PasswordService {
    ...
    pub fn generate_temporary_password() -> String {
        ...
        password.push(rng.random_range('A'..='Z'));
        password.push(rng.random_range('a'..='z'));
        password.push(rng.random_range('0'..='9'));
        password.push("!@#$%^&*".chars().nth(rng.random_range(0..8)).unwrap());
...
mod tests {
    ...
    fn test_password_hashing() {
        ...
        assert!(!PasswordService::verify_password("WrongCredential", &hash).unwrap());
    }
    
    #[test]
    fn test_password_strength_validation() {
        // Valid password
        assert!(PasswordService::validate_password_strength("SecureP@ssw0rd!").is_ok());
        
        // Too short
        assert!(PasswordService::validate_password_strength("Test1!").is_err());
        
        // Missing uppercase
        assert!(PasswordService::validate_password_strength("securecredent1!").is_err());
        
        // Missing lowercase
        assert!(PasswordService::validate_password_strength("SECURECREDENT1!").is_err());
        
        // Missing digit
        assert!(PasswordService::validate_password_strength("SecureCredent!").is_err());
        
        // Missing special character
        assert!(PasswordService::validate_password_strength("SecureCredent123").is_err());
