...
pub fn validate_secrets() -> Result<()> {
        ...
        match env::var(name) {
            Ok(value) => {
                if value.is_empty() {
                    ...
                    weak_secrets.push(format!("{} (minimum {} characters required)", name, min_length));
    ...
    for (name, min_length) in optional_secrets {
        if let Ok(value) = env::var(name) {
            if !value.is_empty() && value.len() < min_length {
                warnings.push(format!("{} is shorter than recommended ({} chars)", name, min_length));
            }
        }
    }
    ...
    if !missing_secrets.is_empty() {
        bail!("❌ Missing required secrets: {}", missing_secrets.join(", "));
    }
...
fn is_weak_secret(secret: &str) -> bool {
    ...
    weak_patterns.iter().any(|pattern| lowercase.contains(pattern))
}
        ...
        if env_type == "production" {
            if let Ok(db_url) = env::var("DATABASE_URL") {
                if db_url.contains("localhost") || db_url.contains("127.0.0.1") {
                    warnings.push("DATABASE_URL points to localhost in production environment".to_string());
                }
            }
            
            if let Ok(redis_url) = env::var("REDIS_URL") {
                if redis_url.contains("localhost") || redis_url.contains("127.0.0.1") {
                    warnings.push("REDIS_URL points to localhost in production environment".to_string());
                }
            }
        }
    }
...
fn validate_ssl_configuration(warnings: &mut Vec<String>) {
    ...
    if let Ok(db_url) = env::var("DATABASE_URL") {
        if !db_url.contains("sslmode=") {
            warnings.push("DATABASE_URL does not specify SSL mode (add ?sslmode=require for production)".to_string());
        } else if db_url.contains("sslmode=disable") || db_url.contains("sslmode=prefer") {
            warnings.push("DATABASE_URL has weak SSL mode (use sslmode=require, verify-ca, or verify-full)".to_string());
        }
    }
    ...
    if let Ok(redis_url) = env::var("REDIS_URL") {
        if !redis_url.starts_with("rediss://") {
            warnings.push("REDIS_URL does not use TLS (consider using rediss:// protocol)".to_string());
        }
